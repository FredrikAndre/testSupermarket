...
function generateCartDropDown(){
    ...
    $.each(cart, (i, items) => {
        $("#droplist").append('<li><div><img width="100" height="70" src="'+(items.photo).replace('..','.')+'"></div><div><a href="#" class="cart-link">'+items.title+'</div><div>'+items.price+'kr</div></a></div><a class="close-btn" onclick="removeFromCart('+i+')"><i class="fa fa-times"></i></a></li>')
    }); 
    if(totalValue > 0){
